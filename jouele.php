<?php
// Jouele plugin, https://github.com/sashatravkina/yellow-plugin-jouele
...
class YellowJouele {
    ...
    public function onLoad($yellow) {
        $this->yellow = $yellow;
        $this->yellow->config->setDefault("joueleStyle", "");
		$this->yellow->config->setDefault("joueleDir", "media/audio/");
    }
    ...
    public function onParseContentBlock($page, $name, $text, $shortcut) {
        ...
        if ($name=="jouele" && $shortcut) {
            list($pattern, $style) = $this->yellow->toolbox->getTextArgs($text);

			if(empty($pattern)) {
				$files = $page->getFiles(true);
			} else {
				$joueles = $this->yellow->config->get("joueleDir");
				$files =  $this->yellow->files->index(true, true)->match("#$joueles$pattern#");
            }

            if(count($files)) {
                $page->setLastModified($files->getModified());

                if (empty($style)) $style = $this->yellow->config->get("joueleStyle");
				$output .= "<div class=\"jouele-playlist ".htmlspecialchars($style)."\">";
					foreach($files as $file) {
						$tags = jouele_get_tags($file->getUrl());
						$count++; $output .= "<p><a href=\"".htmlspecialchars($file->getLocation())."\" class=\"jouele\" data-length=".$tags["formatted_time"]." data-space-control=\"true\">".$count.". ".$tags["artist"]." - ".$tags["title"]."</a></p>";
					}
				$output .= "</div>";
			} else {
	...
	/* 	Jouele
		...
		Created By Ilya Birman
		ilyabirman.net, ilyabirman.ru
		ilyabirman.net/projects/jouele
		Version 3.0.6 - July 13, 2018
	*/
    public function onParsePageExtra($page, $name) {
        $output = null;
        if ($name=="header") {
			$pluginLocation = $this->yellow->config->get("serverBase").$this->yellow->config->get("pluginLocation");
			$output = "<link rel=\"stylesheet\" type=\"text/css\" media=\"all\" href=\"{$pluginLocation}jouele.css\" />\n";
            $output .= "<script type=\"text/javascript\" defer=\"defer\" src=\"{$pluginLocation}jouele.js\"></script>\n";
        }
        return $output;
    }
}
...
function jouele_get_tags($file)
{
	...
	$versions = array("00" => "2.5", "01" => "x", "10" => "2", "11" => "1"); // MPEG Jouele version ID
	$layers = array("00" => "x", "01" => "3", "10" => "2", "11" => "1"); // MPEG Jouele layer description
	$bitrates = array(
	...
	if (!$handle) return; // Version 2.0
	{
		...
		if (substr($data,0,3) == 'ID3') // If first 3 bytes == "ID3"
		{
			...
			if ($id3_major_version >= 3) // Version 2.0c
				...
				$tags_array = array( // second ID3v2.2 tags - http://id3.org/id3v2-00
							'TT2' => 'title', 'TAL' => 'album', 'TP1' => 'artist',
							'TYE' => 'year', 'COM' => 'comment', 'TCO' => 'genre', 'TLE' => 'length'
							);			
